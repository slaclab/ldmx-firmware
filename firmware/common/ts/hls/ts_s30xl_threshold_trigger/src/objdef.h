...
inline void clearClus(Cluster& c) {
    clearHit(c.Seed);
    clearHit(c.Sec);
    c.Cent = -1;  // clearHit(c.For);
}
inline void calcCent(Cluster& c) {
    if (c.Seed.Amp > 0) {
        c.Cent = (ap_int<12>)(10. * ((float)(c.Seed.Amp * c.Seed.bID + c.Sec.Amp * c.Sec.bID)) /
...
inline void calcResid(Track& c) {
    ...
    float mean  = (one + two + three) / 3.0;
    c.resid     = (ap_int<12>)((
        int)(((one - mean) * (one - mean) + (two - mean) * (two - mean) + (three - mean) * (three - mean)) / 3.0));
}
