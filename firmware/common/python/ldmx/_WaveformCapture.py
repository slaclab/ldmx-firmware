...
class WaveformCapture(pr.Device):
    def __init__(self, apvs_per_hybrid, **kwargs):
...
#            bitSize = 32*8,
        ...
        for i in range(apvs_per_hybrid):
            self.add(pr.LinkVariable(
                name = f'V[{i}]',
                guiGroup='AdcAverage',
                disp = '{:0.06f}',
...
class WaveformCaptureReceiver(pr.Device, rogue.interfaces.stream.Slave):
    ...
    def __init__(self, apvs_per_hybrid, num_hybrids, **kwargs):
        rogue.interfaces.stream.Slave.__init__(self)
        pr.Device.__init__(self, **kwargs)
        ...
        self.add(pr.LocalVariable(
            name = 'RawData',
            value = {ch: {'ADC Counts': tmpAdc[ch],
                          'V@ADC': tmpVoltage[ch], } for ch in range(8)},
            mode = 'RO',
            groups = ['NoStream'],
            hidden = True))
        ...
        self.apvs_per_hybrid = apvs_per_hybrid
    ...
    def _acceptFrame(self, frame):
        ...
        # Process header
        apv_num = frame[0] & 0b111
        all_apvs = (frame[0] >> 3) & 0b1
        self.apvs_per_hybrid = (frame[0] >> 11) & 0b111
        hybrid_num = (frame[0] >> 8) & 0b111
        decimation = frame[1]
        ...
        with self.root.updateGroup():
            if all_apvs == 1:
                ...
                adcs.resize(adcs.size//self.apvs_per_hybrid, self.apvs_per_hybrid)
                self.RmsNoiseRaw.set(adcs.std(0))
            else:
                self.RmsNoiseRaw.set(value=adcs.std(), index=apv_num)
            ...
            if all_apvs == 1:
                ...
                print(f'Setting data for apv_num {apv_num}')
                d[apv_num]['ADC Counts'] = adcs
                d[apv_num]['V@ADC'] = voltages
...
class MultiPlot(pr.LinkVariable):
    ...
    def linkedGet(self, read, index=-1):
        ...
        apvs_per_hybrid = self.parent.apvs_per_hybrid

        if self.parent.PlotAPV.getDisp(read=read) == 'All':
            index = 1
            for ch in range(apvs_per_hybrid):
                for func in enabled_plot_functions.values():
                    ax = self.fig.add_subplot(apvs_per_hybrid, num_plots, index)
                    src = func[1].getDisp(read=read)
                    func[0](ch, ax, data[ch][src], src, multi_channel=True)
                    index += 1
        else:
            ch = self.parent.PlotAPV.get(read=read)
